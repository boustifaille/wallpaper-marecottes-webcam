...
# Source - https://stackoverflow.com/a/21213358
# Posted by Martin Hansen, modified by community. See post 'Timeline' for change history
    ...
    def set_wallpaper(self,file_loc, first_run):
            ...
            try:
                ...
                ## see http://blog.zx2c4.com/699 for a solution that might work
                    ...
                    # List all properties in xfce4-desktop
                    props_output = subprocess.check_output(
                    ...
                    if not monitor_props:
                        # fallback for older XFCE versions
                        monitor_props = ["/backdrop/screen0/monitor0/workspace0/last-image"]

                    if first_run:
                        for prop in monitor_props:
                            subprocess.run(
                                ["xfconf-query", "-c", "xfce4-desktop", "-p", prop, "-s", file_loc],
                                check=False
                            )

                    # Reload the desktop to apply wallpaper
